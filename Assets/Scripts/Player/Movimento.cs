...
public class Movimento : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (transform.position.x <= 3.85f && transform.position.x >= -3.85f && !LimiteDireta && !LimiteEsquerda) 
        ...
        else if (transform.position.x >= 3.85f) LimiteDireta = true;
        else if (transform.position.x <= -3.85f) LimiteEsquerda = true;
    }
    ...
    private void FixedUpdate()
    {
        ...
        if(LimiteDireta)
        {
            transform.position = Vector2.MoveTowards(transform.position, new Vector2(3f, transform.position.y), .07f);

            if (transform.position.x == 3f) LimiteDireta = false;
        }
        ...
        if (LimiteEsquerda)
        {
            ...
            if (transform.position.x == -3f) LimiteEsquerda = false;
        }
    }
